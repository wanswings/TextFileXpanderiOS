...
- (void)changeNotification:(NSNotification *)aNotification
{
    ...
    self.naviBarObj.frame = [self getNaviBarRect];
}

- (CGRect) getNaviBarRect
{
    long naviHeight = 64;
    long naviWidth = [[UIScreen mainScreen] bounds].size.width;

    if ( [[UIDevice currentDevice].systemVersion floatValue] < 8.0) {
        UIInterfaceOrientation orientation = [[UIApplication sharedApplication] statusBarOrientation];
        if (orientation == UIInterfaceOrientationLandscapeLeft || orientation == UIInterfaceOrientationLandscapeRight) {
            naviWidth = [[UIScreen mainScreen] bounds].size.height;
        }
    }

    return CGRectMake(0, 0, naviWidth, naviHeight);
}
...
- (void)viewDidLoad {
  ...
  [self setUpNavigation];
    ...
    self.naviBarObj = [[UINavigationBar alloc] initWithFrame:[self getNaviBarRect]];
    [self.view addSubview:self.naviBarObj];

    UIBarButtonItem *cancelItem = [[UIBarButtonItem alloc]initWithTitle:[NSString stringWithFormat:NSLocalizedString(@"Cancel", nil)]
                                                                  style:UIBarButtonItemStyleBordered target:self
                                                                 action:@selector(cancelGdriveSignIn:)];
    UINavigationItem *naviItem = [[UINavigationItem alloc] initWithTitle:@"Google Drive"];
    naviItem.rightBarButtonItem = cancelItem;
    self.naviBarObj.items = [NSArray arrayWithObjects: naviItem,nil];
}
...
//  UINavigationBar *navigationBar = self.navigationController.navigationBar;
  CGRectDivide(webFrame, &dontCare, &webFrame, 64, CGRectMinYEdge);
